...
test_that("read_keds works without one a day", {
  ...
  expect_s3_class(dd, 'eventdata')
  expect_s3_class(dd$date, 'Date')
  expect_s3_class(dd$source, 'factor')
  expect_s3_class(dd$target, 'factor')
  expect_s3_class(dd$code, 'factor')
...
test_that("read_keds works with multiple files", {
  dd <- read_keds(c('./tabari-demo.keds', './tabari-demo.keds'), 
                  one.a.day = TRUE)
