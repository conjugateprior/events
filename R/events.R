...
how_many_keds_doc_lines_to_skip <- function(fname, max = 500) {
  ll <- readLines(fname, n = max)
  mat <- grep("DOC\tDOC\t999", ll)
  if (length(mat) > 0)
    max(mat)
  else
    0
...
read_eventdata <- function(d, col.format = "D.STC", one.a.day = TRUE,
    ...
    if (sum(c(date.col = -1, source.col = -1, target.col = -1, 
              code.col = -1)) > 0)
    ...
    read_ed_file <- function(d) {
      sk <- how_many_keds_doc_lines_to_skip(d)
      vv <- read.csv(d, sep = sep, header = head, strip.white = TRUE,
                     colClasses = "character", skip = sk)
      dd <- data.frame(
        date = vv[, date.col],
...
##' @export
##' @author Will Lowe
read_keds <- function(d, keep.quote = FALSE, keep.label = TRUE,
    ...
    form <- paste("DSTC", 
                  ...
                  ifelse(keep.quote, "Q", ""),
                  sep = '')
    read_eventdata(d, col.format = form, one.a.day = one.a.day,
...
##' @param fun Function that returns \code{TRUE} for actor codes that should 
##'            not be discarded.
...
##' @export
##' @author Will Lowe
filter_codes <- function(edo, fun = function(x) TRUE) {
  filter_eventdata(edo, fun, 'code')
}
...
##' @export
##' @author Will Lowe
map_codes <- function(edo, fun = function(x) x) {
  if (is.list(fun))
...
##' @export
##' @author Will Lowe
map_actors <- function(edo, fun = function(x) x) {
...
make_dyads <- function(edo, scale = NULL,
    ...
    ff <- function(a, b) {
      ...
      if (is.null(scale)) {
        ...
        agg <- aggregate(edo.subset[[scale]],
                         by = list(unit = edo.subset$sEGs),
                         FUN = fun)
        hh <- data.frame(date = as.Date(agg$unit))
        hh[[scale]] <- agg$x
        aggn <- aggregate(edo.subset[[scale]],
                          by = list(unit = edo.subset$sEGs),
                          FUN = length) ## to get N
        hh$n <- aggn$x
...
##' @importFrom utils read.csv
##' @author Will Lowe
make_scale <- function(name, types = NULL, values = NULL, file = NULL,
                       desc = "", default = NA, sep = ",") {
  def <- default
  v <- list()
  if (!is.null(file)) {
...
summary.eventscale <- function(object, ...) {
  ...
  cat(paste(len.es, "event codes assigned scores between",
            min.es, "and", max.es, "\n"))
}
...
fisher.transform <- function(x, absminmax = 1) {
  if (absminmax != 1)
    x <- x/absminmax
  0.5 * log((1 + x) / (1 - x))
}

## assumes [-1,1] ranged original data, so inflate afterwards
inv.fisher.transform <- function(x, absminmax = 1) {
...
#'
#' @param x an event scale 
#' @param row.names ignored
#' @param optional ignored
#' @param ... ignored
#'
#' This function converts a list with event codes as names and event scores 
#' as values into a data frame with column 'code' containing the event codes
#' and column 'score' as the event's score
#'
